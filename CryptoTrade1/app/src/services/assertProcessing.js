...
export async function assertProcessing(portfolio, coins, setPortfolioTransactions, setPortfolioAssets, setIsLoading) {
    try {
            ...
            if (portfolio && coins.length > 0) {
                ...
                // Створення об'єкту конфігурації для запиту Axios
                const config = {
                    headers: {
                ...
                // Отримання транзакцій з сервера за допомогою Axios з використанням токена авторизації
                const response = await axios.get(`${apiUrl}/api/transactions/${portfolioId}`, config);
